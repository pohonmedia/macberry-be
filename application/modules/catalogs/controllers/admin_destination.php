 ...
 */
class Admin_destination extends Admin_Controller {
    ...
    public function __construct() {
        parent::__construct();
        //LOAD MODEL
        $this->load->model('Catalogs_destination_m');
        $this->_db = $this->Catalogs_destination_m;
        //SLUG LIBRARY CONFIG
        $config = array(
            'field' => 'type_slug',
            'title' => 'type_name',
        ...
        $this->data['subnav_active'] = 'destination';

        $this->breadcrumbs->push('Catalogs', 'admin/catalogs');
        $this->breadcrumbs->push('Destination', 'admin/catalogs/destination');
    }

    public function index() {
        $this->set_title('Package Destination');
        $param = array();
        if ($this->input->post('catalogs_destination_search') != "") {
            $param[] = array('field' => 'destination_name', 'param' => '', 'operator' => 'LIKE', 'value' => '%' . $this->input->post('catalogs_destination_search') . '%');
            $this->data['search'] = $this->input->post('catalogs_destination_search');
        }
        //PAGING OPTION
        $total_row = $this->_db->count_all($param);
        $limit = 10;
        $this->data['page'] = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
        $this->_paginate('admin/catalogs/destination', $total_row, $limit, 4);

        $this->data['list'] = $this->_db->get_all($param, 'ASC', $limit, $this->data['page']);
        $this->data['count_data'] = $total_row;
        $this->data['page_desc'] = 'Package Destination Management';
        $this->data['msg'] = $this->session->flashdata('msg');

        $this->template->build('destination/admin/index', $this->data);
    }
