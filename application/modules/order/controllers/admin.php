...
class Admin extends Admin_Controller {
    ...
    public function index() {
        ...
        $param = array();
        if ($this->input->post('order_search') != "") {
            $param[] = array('field' => 'vcordercode', 'param' => '', 'operator' => 'LIKE', 'value' => '%' . $this->input->post('products_search') . '%');
            $this->data['search'] = $this->input->post('order_search');
        }
        //PAGING OPTION
        $total_row = $this->_db->count_all($param);
        $limit = 10;
        $this->data['page'] = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $this->_paginate('admin/order', $total_row, $limit);
        $data_order = $this->_db->get_all($param, null, $limit, $this->data['page']);
        // if (!empty($data_order)) {
        //     foreach ($data_order as $key => $value) {
        //         $get_img = $this->_db->get_thumb($value->id);
        //         $data_order[$key]->img_thumb = $get_img == null ? 'assets/modules/catalogs/default-image.jpg' : $get_img->prod_thumb_url;
        //     }
        // }
        $this->data['list'] = $data_order;
        $this->data['count_data'] = $total_row;
        $this->data['page_desc'] = 'Order Management';
    ...
    public function detail($id) {
        $this->set_title('Detail Order');
        $this->breadcrumbs->push('Detail', 'admin/order/detail/' . $id);

        $order = $this->_db->get_detail('MO.id', $id);
        $order_detail = $this->_db->get_detail_order('OD.intorderid', $id);

        $this->data['order'] = $order;
        $this->data['order_detail'] = $order_detail;

        $this->template->build('admin/detail', $this->data);
    }
